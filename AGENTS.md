...
> Mirror of `CLAUDE.md` plus the **Conventions** section (which lives only here). When editing project guidance, edit `CLAUDE.md` first, then regenerate this file — last synced 2026-07-01.

## Project Overview
Personal tool that watches for live reaction waves around recognizable fictional characters (anime, gacha, games) on Reddit, identifies the audience's unmet desire (wish-fulfillment or satire), and generates full content packages (video + script + captions + thumbnail + hashtags) using AI. TikTok scraping (apidojo) built a separate Blueprint/RAG training corpus — a one-time archive ingest (done 2026-06-09), dormant, not part of the live content loop.
...
**OpenMontage harvest rule: for any non-trivial task touching `src/generation/`, `src/monitor/`, or a new pipeline/architecture decision — check `ai_video_resources/OpenMontage/PROJECT_CONTEXT.md` at start (does a pattern already exist there worth adapting?) and again before marking the task done (did the work just done duplicate something OpenMontage already solved better?).** Look for: artifact-contract discipline (schema-validated stage in/out), the selector+provider tool-registry pattern, cost-governance (estimate before spend), checkpoint/resume. Note and adapt, don't blindly port — OpenMontage is a human-driven synchronous coding-agent session (prose skills execute the live agent's own tools); our monitor/writer/executor stack runs unattended, so anything that assumes a human turn-by-turn chat doesn't transfer. Skip this check for trivial one-line fixes.

**Status/history rule: for ANY "where are we" / "what did we decide" / "is X still true" question, query claude-mem (`ToolSearch` → `mcp__plugin_claude-mem_mcp-search__search` / `timeline`) BEFORE trusting git log or current file state.** `docs/superpowers/specs/` and `docs/superpowers/plans/` are gitignored — design pivots (e.g. the 2026-06-26/06-27 content-strategy supersessions) leave **zero git trace**; the only record is claude-mem or the doc's own content. git log proves code landed, never that a decision was made, reversed, or superseded. Also check a doc's own header for a `SUPERSEDED` marker — specs chain-supersede each other; only the most recent unsuperseded one is true. Don't guess from git/grep when claude-mem can answer directly. (Agents without claude-mem access: the principle still applies — check spec headers for SUPERSEDED markers and never infer decision history from git log alone.)
...
uv run python -m scripts.pitch_angles --dry-run                    # LIVE loop: scrape -> gate -> gap -> pitch slate (Path A)
uv run python -m scripts.pitch_angles --topic "<topic>"            # LIVE loop: user-topic on-ramp via context agent (Path B)
...
- **LLM / Structured output**: `anthropic` + `openai` SDKs; structured output via Anthropic native `messages.parse` (`src/providers/llm/anthropic_llm.py`). `instructor` is installed but no longer in the call path.
...
- Providers configured via `config/providers.yaml` — swapping = YAML edit, no code change. STATUS: wired today only for embedder/reranker; LLM model ids and the Higgsfield CLI are still hardcoded at call sites (see audit AUD-M21).
- 3-step approval workflow: approve trend → approve prompts → approve generated content
- Blueprint Foundation (P1.5): every scraped video gets a structured Blueprint extracted by Claude Sonnet via native structured output (`messages.parse`). Stored in versioned `blueprints` table; conditions RAG retrieval, drives generation, feeds ML features. CI gate: `schema_valid_rate ≥ 0.95`. See **Blueprint versioning** in Conventions for version history.
- Closed-loop (P3.5): generate AI videos → post to TikTok test account → track 7-day views → write `outcome_view_percentile` back → P4 labels. Recording scripts built (`record_post` / `enter_views` / `compute_percentiles`); render executor (`src/generation/executor.py`) BUILT and smoke-confirmed (2026-06-23, 24cr) — unaffected by the content-strategy pivots below. **Content-strategy spec chain (premise source): 2026-06-23 broad-Reddit → SUPERSEDED 2026-06-26 by single-niche anime/gacha → SUPERSEDED 2026-06-27 by reaction-driven (current, not superseded).** Current spec (`docs/superpowers/specs/2026-06-27-reaction-driven-content-system-design.md`): watch for a live reaction wave around a recognizable FICTIONAL character/IP (any niche, not locked to one — the account coheres on audience+format, not a single art style), render the thing the reaction is begging to see (wish-fulfillment OR satire), as a reference-grounded multi-shot (3–6 clips, ~12–25s) narrated/captioned video; real-actor likeness deferred. **Implementation is mid-migration, not fully caught up to the current spec:** `src/monitor/idea_fit_gate.py` matches it (recency hard sub-gate + wish/satire `mode` classification). `gap_agent.py` and `angle_pitcher.py` are still on the older 06-23 schema (8-way `GapType` enum, 3-tableau pitch) — not yet rewritten to the mode-aware story-craft schema (protagonist/beats/hook→turn→payoff) the current spec calls for. The context-gathering agent (LangGraph + Tavily/Reddit tools) is BUILT and wired: `scripts/pitch_angles.py --topic` (Path B) runs it end-to-end, persisting the bundle to `trending_events.context_bundle`. **The loop ran end-to-end live once (2026-06-26)** — but on the OLD broad-Reddit/tableau pipeline, predating both supersessions; not yet re-validated against the current spec. **See PLAN.md "Current Reality" for live status** (refreshed 2026-06-30 with the spec-chain + mid-migration state).
...
- `src/generation/` — (P3) `content_writer.py` turns a **premise** (+ optional RAG winners for style/lane) into a ContentPackage; `executor.py` runs the render jobs (still → i2v → download); `render_adapters/` declares per-shot Higgsfield jobs (reads `config/render_rules.yaml`). Built for **single-shot** delivery — the current reaction-driven spec (06-27) targets **multi-shot assembly** (3–6 clips, ~12–25s); executor/adapters not yet extended for that, a known gap. `premise_generator.py` (grounded) and the imagination path were superseded by news-reactive premises from `src/monitor/` (BUILT, mid-migration — see Architecture) and deleted (commit 6087fea, 2026-07-04). Safety module still planned.
- `src/monitor/` — (P3.5 reaction-driven front-end, BUILT, mid-migration) Reddit scraper → event extractor → **idea-fit gate (current-spec)** → gap agent → angle pitcher (**both still old-schema, due a story-craft rewrite**) → format router; feeds approved angles to the writer via `scripts/pitch_angles.py`. Context-gathering agent (LangGraph + Tavily/Reddit): BUILT, wired via `--topic` (Path B). Spec chain: 2026-06-23 → 2026-06-26 → **2026-06-27 (current)**, see Architecture.
- `src/models/` — SQLAlchemy ORM models
- `src/schemas/` — Pydantic request/response schemas
- `src/observability/` — Langfuse tracing (`tracing.py`)
- `ui/` — React dashboard (Vite + TypeScript) — PLANNED; directory is currently an empty skeleton (no package.json, no source)
...
- `docs/adr/` — architecture decision records (7 ADRs; schema locks, Postgres-only, extractor cache policy, cost governance)
...
- **Content strategy (spec chain, see Architecture):** P3.5 publishing pivoted 2026-06-23 from imagination-led `surreal_hyperreal` premises to news-reactive content, then superseded 2026-06-26 (single anime/gacha niche) and again 2026-06-27 → **current: reaction-driven** (recognizable fictional character mid-reaction-wave, wish-fulfillment or satire, multi-shot). `surreal_hyperreal` stays as a dormant fallback AND remains the Blueprint/extractor test-fixture niche (P1.5 unaffected). Current spec: `docs/superpowers/specs/2026-06-27-reaction-driven-content-system-design.md`.
